...
class StmgrDecoder(object):
    ...
    def build_fields(self, line):
        ...
        if match_obj is not None:
            ...
            level = self.conf['define']['level'].get(level_msg, 8)
            ...
            self.status_set = self.conf['define']['status']
...
class Outputer(InfluxDBBase):
    def __init__(self, conf, processor):
        ...
        self.url = conf[processor]["config"]["url"]
        self.nodename = conf[processor]["config"]["nodename"]
        self.eqpt_no = conf[processor]["config"]["eqpt_no"]
        self.parser = StmgrDecoder(conf)
