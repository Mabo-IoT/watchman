...
class Watcher(object):
    ...
    def __init__(self, item, app_conf):
        ...
        self.splide = watcher_conf["splide"]
        self.splide_num = watcher_conf["num"]
    ...
    def _watch(self):
        ...
        for filename in self.matched_files:
            ...
            contents_ = self.splide_log(contents)
            length = len(contents_)
            log.debug(length)
            while length > 0:
                message = contents_[length - 1]
                log.debug(length)
                log.debug(message)
    ...
    def read_contents(self, filename):
        ...
        present_point = self.get_seek(filename)
        log.debug(filename)

        if file_size == present_point:
            log.debug('file_size = present_point',fn)
            return 'pass'
